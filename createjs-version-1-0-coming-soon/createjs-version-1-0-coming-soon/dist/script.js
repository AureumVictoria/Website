...
function createBG(total, cols, width, height) {
  ...
  for(var i=1;i<total;i++) {
    ...
    line.graphics.f("#d16500").dr(0, 0, 2, 1);
  ...
  for(i=1;i<(total*4);i++) {
    line = new createjs.Shape();
    line.graphics.f("#d16500").dr(0, 0, 1, 2);
    line.y = (i/cols|0) * (1+paddingY);
    line.scaleX = 1;
    line.rowIndex = i;
...
function createLogo() {
  ...
  tri.width = triW;
  tri.height = triH;
  tri.graphics.s("#d16500").ss(3,2,0).mt(0, 0);
  tri.graphics.lf(["#000000","#d16500"], [.1, 1], 60, 30, 10, 220).mt(0, 0);
...
function createPanel(w, h) {
  ...
  s.width = w;
  s.height = h;
  s.graphics.f("#d16500").dr(0, 0, w, h);
...
function updateGrid(stage, w, h) {
  ...
  square.graphics.ss(4).s("#d16500").mt(0,0).lt(w, 0).lt(w, h).lt(0, h).lt(0, 0);
